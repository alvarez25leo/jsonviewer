...
import { isContentImage } from "@/lib/utils/graph/calculateNodeSize"
	...
	return (
		<Styled.StyledForeignObject width={width} height={height} x={0} y={0}>
			{isImage ? (
				...
				<StyledTextNodeWrapper data-x={x} data-y={y} data-key={JSON.stringify(text)} $hasCollapse={isParent && collapseButtonVisible}>
...
function propsAreEqual(prev: CustomNodeProps, next: CustomNodeProps) {
	return prev.node.text === next.node.text && prev.node.width === next.node.width && prev.node.data.childrenCount === next.node.data.childrenCount
}
