...
import { getChildrenEdges } from "@/lib/utils/graph/getChildrenEdges"
import { getOutgoers } from "@/lib/utils/graph/getOutgoers"
import { EdgeData, NodeData } from "@/types"
import { parser } from "@/lib/utils/json/jsonParser"
...
const useGraph = create<Graph & GraphActions>((set, get) => ({
	...
	expandNodes: (nodeId) => {
		const [childrenNodes, matchingNodes] = getOutgoers(nodeId, get().nodes, get().edges, get().collapsedParents)
	...
	collapseGraph: () => {
		...
		const collapsedNodes = get()
			.nodes.filter((node) => !parentNodesIds.includes(node.id) && !secondDegreeNodesIds.includes(node.id))
			.map((node) => node.id)
	...
	centerView: () => {
		...
		const canvas = document.querySelector(".draw-graph") as HTMLElement | null
		if (canvas) {
